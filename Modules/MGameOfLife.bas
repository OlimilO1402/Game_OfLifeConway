...
Public Sub Clear(this As TGameOfLife)
    ...
    End With
End Sub
Public Sub SwitchLife(this As TGameOfLife, IndexPt As Point)
            ...
            For j = 1 To .SizeY
                For i = 1 To .SizeX
                    With .pThis.Generation
                        ...
                        If .Arr(i - 1, j - 1).Life = True Then neighbours = neighbours + 1
                        ' den darüber prüfen
                        If .Arr(i, j - 1).Life = True Then neighbours = neighbours + 1
                        ' den rechts oben prüfen
                        If .Arr(i + 1, j - 1).Life = True Then neighbours = neighbours + 1
                        ' den links prüfen
                        If .Arr(i - 1, j).Life = True Then neighbours = neighbours + 1
                        ' den rechts prüfen
                        If .Arr(i + 1, j).Life = True Then neighbours = neighbours + 1
